...
export default class Enemy {
    constructor(image, a, b) {
        ...
        this.generalWeapon = new Weapon();
        this.x = a;
        this.y = b;
        this.img = this.startVars.pjs.tiles.newImage(image);
        ...
        this.speed = 2;
        this.weapon;
        this.shotPoint = this.startVars.game.newCircleObject({
            fillColor: "black",
            radius: 3
        });
        ...
        this.int = setInterval(function () {
            ...
            for (let i = 1; i < 4; i++) {
                setTimeout(function () {
                    let bull = this.startVars.game.newImageObject({
                    ...
                    bull.damage = 1;
                    bull.speed = 20;
                    this.generalWeapon.enemyBulls.push(bull);
                }, 100 * i);
            }
        }.bind(this), 2000);
    }
    ...
    draw() {
        ...
        if (this.health > 0) {
            ...
            if (this.startVars.oPos.x - this.obj.getPositionC().x >= 0) {
            ...
            this.weapon.rotate(this.startVars.oPos);
            this.weapon.draw();

            this.shotPoint.setPositionC(this.obj.getPositionC());
            this.shotPoint.rotate(this.startVars.oPos);

        } else {
            clearInterval(this.int);
            self.w = 150;
            self.h = 150;
            self.setAnimation(deathAnim); //TODO deathAnim
            self.setDelay(5);
            self.draw();
            setTimeout(function () {
                self.visible = false;
    ...
    init() {
        let self = this;
        let an = this.anim;
        this.obj = this.startVars.game.newAnimationObject({
            x: self.x,
            y: self.y,
            animation: an,
            w: 72,
        ...
        this.weapon = this.startVars.game.newImageObject({
